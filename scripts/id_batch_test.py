...
THRESHOLD      = float(os.environ.get('THRESHOLD', '0.8'))      # 單張通過門檻
TARGET_ACC     = float(os.environ.get('TARGET_ACC', '0.8'))     # 整體門檻
BASELINE_INDEX = int(os.environ.get('BASELINE_INDEX', '-1'))    # -1=自動找
USE_VISION     = os.environ.get('USE_VISION', '1') == '1'       # 先用 Vision 裁臉
# ===================================
...
def to_rgb_jpeg_bytes(img_bytes: bytes) -> bytes:
    """任何格式 → 8-bit RGB JPEG（面向 dlib/face_recognition 相容）"""
...
def crop_face_vision(img_bytes: bytes) -> bytes:
    """用 Cloud Vision 抓第一張臉並裁切；找不到臉就回原圖。"""
    if not USE_VISION:
        return img_bytes
    try:
...
def embed(img_bytes: bytes) -> Optional[np.ndarray]:
    """以 face_recognition 產生 128D 向量；先轉 RGB JPG 再讀檔，避免類型問題。"""
    import face_recognition
    try:
        jpg = to_rgb_jpeg_bytes(img_bytes)
        with tempfile.NamedTemporaryFile(suffix='.jpg') as tmp:
            tmp.write(jpg); tmp.flush()
            arr = face_recognition.load_image_file(tmp.name)   # 確保 dtype=uint8, shape(H,W,3)
...
def main() -> int:
    ...
    # baseline：指定 index 或自動找第一張能取到 embedding 的
    ...
    for p in use_imgs:
        ...
        if e is None:
            sims.append(0.0); log(f"{p.name} sim=0.000 pass=False (no_face)"); continue
        s = cos(baseline, e); sims.append(s)
